...
class WMP_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		// echo apply_filters( 'wp_most_popular_list_before', '<ul class="most-popular-posts">' );
		echo apply_filters( 'wp_most_popular_list_before', '<div class="most-popular-posts articleMiniPreview ">' );

		global $post;

		function append_ad_code($item) {
			$ad_html_top = '
			<script type=\'text/javascript\'><!--//<![CDATA[
			   var m3_u = (location.protocol==\'https:\'?\'https://ads.oxfordclub.com/www/delivery/ajs.php\':\'http://ads.oxfordclub.com/www/delivery/ajs.php\');
			   var m3_r = Math.floor(Math.random()*99999999999);
			   if (!document.MAX_used) document.MAX_used = \',\';
			   document.write ("<scr"+"ipt type=\'text/javascript\' src=\'"+m3_u);
			   document.write ("?zoneid=244");
			   document.write (\'&amp;cb=\' + m3_r);
				 document.write (\'&amp;segments=\' + \''. preg_replace("/[^a-zA-Z0-9,]/", "", $_COOKIE['ly_segs']) .'\');
			...
			//]]>--></script>';
			$item = $item . $ad_html_top;
			return $item;
		}

		function append_bottom_ad_code($item) {
			$ad_html_bottom = '
			<script type=\'text/javascript\'><!--//<![CDATA[
			   var m3_u = (location.protocol==\'https:\'?\'https://ads.oxfordclub.com/www/delivery/ajs.php\':\'http://ads.oxfordclub.com/www/delivery/ajs.php\');
			   var m3_r = Math.floor(Math.random()*99999999999);
		...
		foreach ( $posts as $post ):
			...
			// Top Ad
			if ($count == 4) {
				add_filter('wp_most_popular_list_item_single','append_ad_code');
			}
			else{
				remove_filter('wp_most_popular_list_item_single','append_ad_code');
			}
			// Bottom Ad
			if ($count == 20) {
				add_filter('wp_most_popular_list_item_single','append_bottom_ad_code');
			}
			else{
				remove_filter('wp_most_popular_list_item_single','append_bottom_ad_code');
			}
		...
		// endforeach;
		echo apply_filters( 'wp_most_popular_list_after', '</div>' );
		echo $after_widget;
	...
	public static function list_items( $post, $defaults ) {
		...
		if(in_category('trade-of-the-day-plus')) {
			$icon = '<span class="tpu-plus d-none d-md-block">+</span>';
		}elseif(in_category('videos')){
			$icon = '<span class="videoNumber">►</span>';
		}
		if(!in_category('videos') && !is_singular( 'reports' )) {
		$item						= '

			<div class="' . $post_class . ' row row-eq-height">
					 <div class="col-md-5 articleMiniThumbnail" style="padding-right: 15px;">

						<a href="' . $permalink . '"><img src="'.$image_url.'" class="img-fluid" alt="..."></a>
						'. $icon .'

	                 </div>
	                 <div class="col-md-7 articleMiniInfo px-3 px-md-0 py-2 py-md-0">
	                    <a href="' . $permalink . '"><p class="articleMiniTitle">'.$title.'</p></a>
	                 </div>
			</div>
